...
class FacialFeaturesEstimator(object):
    ...
    def align_face(self, rgb_image, landmarks):
        ...
        aligned_face = cv2.warpAffine(rgb_image, H, (96, 96))
        return aligned_face
    ...
    def estimate(self, rgb_image, faces, camera_matrix=None, dist_coeffs=None):
        ...
        for f in faces:
            ...
            if w > 27 and h > 27:
                if self.align is True:
                    aligned_face = self.align_face(rgb_image, f.features["facial_landmarks"])
        ...
        if len(cropped_imgs) > 0:
            ...
            for f, features in zip(faces, self.model.forward()):
                ...
                if self.name not in f.features:
                    f.features[self.name] = Features(self.name, self.dimensions, np.array(features), confidence)
                else:
                    f.features[self.name].update(np.array(features), confidence)
