...
from .reasoning.monitoring.heatmap import HeatmapMonitor

class HeatmapPipeline(BasePipeline):
    def __init__(self):
        """
        """
        super(HeatmapPipeline, self).__init__()
    ...
    def initialize_pipeline(self, internal_simulator, beliefs_base):
        ...
        self.heatmapmonitor = HeatmapMonitor(internal_simulator)
        self.events = []
    ...
    def perception_pipeline(self, view_pose, rgb_image, depth_image=None, time=None):
        ...
        k=self.heatmapmonitor.update_heatmaps(view_pose,self.robot_camera)
